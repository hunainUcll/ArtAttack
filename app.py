...
def get_pipe(control_mode: str = "openpose", lora_path: str = ""):
    global PIPE, CURRENT_CONTROL_MODE

    control_mode = (control_mode or "openpose").lower().strip()

    if PIPE is None or CURRENT_CONTROL_MODE != control_mode:
        PIPE = load_pipeline(
...
def run_generation(
    ...
    pipe = get_pipe(control_mode=control_mode)
...
with gr.Blocks(title="Sketch2DarkFantasy") as demo:
    ...
    with gr.Accordion("Advanced settings", open=False):
        steps = gr.Slider(10, 60, value=35, step=1, label="Inference steps")
        guidance_scale = gr.Slider(1.0, 15.0, value=8.5, step=0.5, label="Guidance scale")
        controlnet_conditioning_scale = gr.Slider(0.2, 2.0, value=0.85, step=0.05, label="ControlNet conditioning scale")
        ...
        control_mode = gr.Dropdown(choices=["openpose", "canny"],value="openpose",label="Control mode")
